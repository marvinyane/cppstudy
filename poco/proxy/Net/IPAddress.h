...
public:
	...
	IPAddress(const void* addr, base_socklen_t length);
		/// Creates an IPAddress from a native internet address.
		/// A pointer to a in_addr or a in6_addr structure may be 
		/// passed.

	IPAddress(const void* addr, base_socklen_t length, base::UInt32 scope);
		/// Creates an IPAddress from a native internet address.
		/// A pointer to a in_addr or a in6_addr structure may be 
    ...
    IPAddress(const Poco::Net::IPAddress& address);
        /// 
